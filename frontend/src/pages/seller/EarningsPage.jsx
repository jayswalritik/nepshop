...
const EarningsPage = () => {
  ...
  const fetchEarnings = async () => {
    ...
    try {
      ...
      // Delivered shipments in period = earned (subject to settlement window)
      const delivered = all.filter(o => o.status === 'delivered' && inPeriod(o));

      // Projected seller earning per shipment = sellerSubtotal − commission
      // (NEVER includes delivery). Used for in-progress shipments, where
      // settlement.sellerShare hasn't been computed yet (it's only set at
      // delivery time, in deliveryController.markDelivered).
      const sellerEarning = (o) => o.sellerSubtotal - o.commissionAmount;

      // Once delivered, settlement.sellerShare is the authoritative, already
      // locked-in earning for that shipment — identical to sellerEarning(o)
      // in the normal case, but the field of record. Falls back to the
      // formula only if a shipment was force-delivered without ever going
      // through settlement (e.g. an admin status override).
      const settledEarning = (o) => o.settlement?.sellerShare || sellerEarning(o);

      const productRevenue = delivered.reduce((sum, o) => sum + o.sellerSubtotal, 0);
      const totalCommission = delivered.reduce((sum, o) => sum + o.commissionAmount, 0);
      const totalEarnings   = delivered.reduce((sum, o) => sum + settledEarning(o), 0);

      // Available vs pending — based on settlement lock
      let available = 0, pending = 0;
      delivered.forEach(o => {
        const earning = settledEarning(o);
        const released = o.settlement?.sellerReleased;
        if (released) available += earning;
        else pending += earning;
      });

      // In-progress shipments (not yet delivered, not yet earned)
  ...
  const handleExport = () => {
    const rows = orders.map(o => ({
      orderId:      '#' + (o.order?._id || o._id).slice(-8).toUpperCase(),
      date:         new Date(o.createdAt).toLocaleDateString('en-NP'),
      productRevenue: o.sellerSubtotal,
      commission:   o.commissionAmount,
      earning:      o.settlement?.sellerShare || (o.sellerSubtotal - o.commissionAmount),
    ...
    <div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 mb-6">
        ...
        <div className="bg-gradient-to-br from-yellow-50 to-amber-50 border border-yellow-200 rounded-xl p-5">
          ...
          <p className="text-xs text-yellow-600 mt-1">Locked during the return window</p>
        </div>
              ...
              {orders.map((order) => (
                <div key={order._id} className="flex items-center justify-between py-2 border-b border-gray-50 last:border-0">
                  ...
                  <div className="text-right">
                    <p className="text-xs text-gray-400">
                      Product Rs {order.sellerSubtotal.toLocaleString()}
                    </p>
                    <p className="text-sm font-semibold text-green-600">
                      You earn Rs {settledEarning(order).toLocaleString()}
                    </p>
                  </div>
        ...
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-indigo-700">
              ...
              After delivery, your earning is held in escrow during <strong>the return window</strong>.
